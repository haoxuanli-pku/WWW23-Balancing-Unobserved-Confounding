...
def para(args): 
    if args.dataset == 'coat':
        args.training_args = {'batch_size': 128, 'epochs': 500, 'patience': 60, 'block_batch': [64, 64],"alpha": 1}
        args.base_model_args = {'emb_dim': 32, 'learning_rate': 1e-3, 'weight_decay': 1e-3}
        args.weight_model_args = {'emb_dim': 32, 'learning_rate': 1e-3, 'weight_decay': 1e-1}
        args.propensity_model_args = {'emb_dim': 32, 'learning_rate': 1e-3, 'weight_decay': 1e-1}
    elif args.dataset == 'music':
        args.training_args = {'batch_size': 1024, 'epochs': 500, 'patience': 60, 'block_batch': [6000, 500],"alpha": 1}
        args.base_model_args = {'emb_dim': 32, 'learning_rate': 1e-3, 'weight_decay': 5e-4}
        args.weight_model_args = {'emb_dim': 32, 'learning_rate': 5e-3, 'weight_decay': 1e-1}
        args.propensity_model_args = {'emb_dim': 32, 'learning_rate': 5e-3, 'weight_decay': 1e-1}
    else: 
...
def train_and_eval(train_data, unif_train_data, val_data, test_data, device = 'cuda', 
        ...
        weight_model_args: dict = {'emb_dim': 10, 'learning_rate': 0.1, 'weight_decay': 0.1}, 
        propensity_model_args: dict = {'emb_dim': 10, 'learning_rate': 0.1, 'weight_decay': 0.1}, 
        training_args: dict =  {'batch_size': 1024, 'epochs': 100, 'patience': 20, 'block_batch': [1000, 100],"alpha": 1}): 
    ...
    weight_model = MF(n_user, n_item, dim= weight_model_args['emb_dim'], dropout=0).to(device)
    weight_optimizer = torch.optim.Adam(weight_model.parameters(), lr=weight_model_args['learning_rate'], weight_decay=weight_model_args['weight_decay'])
    ...
    for epo in range(early_stopping.max_epochs):
        for u_batch_idx, users in enumerate(train_loader.User_loader): 
            for i_batch_idx, items in enumerate(train_loader.Item_loader): 
                ...
                # update weight model
                weight_model.train()
                w = weight_model(users_train, items_train) # compute weight
                w = torch.softmax(w, dim=0)

                y_hat_train = model(users_train, items_train)
                y_hat_train_detach = torch.detach(y_hat_train)
                e_train = none_criterion(y_hat_train_detach , y_train)
                mnar_loss = (1/len(y_train)) * torch.sum(inverse_p * w * e_train)
                ...
                loss_weight_model = alpha * sum_criterion(mnar_loss,mar_loss) - 1/len(w) * torch.dot(w,torch.log(w)) 
                ...
                w = weight_model(users_train, items_train) # compute weight
                w = torch.softmax(w, dim=0)
                w_detach = torch.detach(w)

                loss = torch.sum(inverse_p * w_detach * cost) 
...
if __name__ == "__main__": 
    ...
    train_and_eval(train, unif_train, validation, test, device, model_args = args.base_model_args, training_args = args.training_args, 
                   weight_model_args = args.weight_model_args, propensity_model_args = args.propensity_model_args)
