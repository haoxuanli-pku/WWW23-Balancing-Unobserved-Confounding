...
def para(args): 
    if args.dataset == 'coat':
        ...
        args.base_model_args = {'emb_dim': 10, 'learning_rate': 1e-3, 'weight_decay': 1e-3}
        args.weight_model_args = {'emb_dim': 10, 'learning_rate': 1e-3, 'weight_decay': 1e-3}
    elif args.dataset == 'music':
        args.training_args = {'batch_size': 1024, 'epochs': 500, 'patience': 60, 'block_batch': [6000, 500],"alpha": 1}
        args.base_model_args = {'emb_dim': 10, 'learning_rate': 5e-4, 'weight_decay': 1e-5}
        args.weight_model_args = {'emb_dim': 10, 'learning_rate': 5e-4, 'weight_decay': 1e-5}
    else: 
    ...
    for epo in range(early_stopping.max_epochs):
        for u_batch_idx, users in enumerate(train_loader.User_loader): 
            for i_batch_idx, items in enumerate(train_loader.Item_loader): 
                ...
                mnar_loss = (1/len(y_train)) * torch.sum(w * e_train)
...
if __name__ == "__main__": 
    ...
    train_and_eval(train, unif_train, validation, test, device, model_args = args.base_model_args, training_args = args.training_args,weight_model_args = args.weight_model_args)
