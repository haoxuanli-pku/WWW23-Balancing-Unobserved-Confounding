...
def para(args): 
    if args.dataset == 'coat':
        ...
        args.base_model_args = {'emb_dim': 32, 'learning_rate': 1e-3, 'imputaion_lambda': 1, 'weight_decay': 1e-3}
        args.weight1_model_args = {'learning_rate': 1e-3, 'weight_decay': 1e-1}
        args.weight2_model_args = {'learning_rate': 1e-3, 'weight_decay': 1e-1}
        args.w1_model_args = {'emb_dim': 32, 'learning_rate': 1e-2, 'weight_decay': 1e-2}
        args.w2_model_args = {'emb_dim': 32, 'learning_rate': 1e-2, 'weight_decay': 1e-2}
        args.imputation_model_args = {'learning_rate': 1e-2, 'weight_decay': 1e-3}      
    elif args.dataset == 'music':
        args.training_args = {'batch_size': 1024, 'epochs': 500, 'patience': 60, 'block_batch': [3000, 300],"alpha": 1, "beta": 1}
        args.base_model_args = {'emb_dim': 256, 'learning_rate': 5e-4, 'imputaion_lambda': 1e-2, 'weight_decay': 5e-5}
        args.weight1_model_args = {'learning_rate': 1e-2, 'weight_decay': 1e-3}
        args.weight2_model_args = {'learning_rate': 1e-2, 'weight_decay': 1e-3}
        args.w1_model_args = {'emb_dim': 256, 'learning_rate': 1e-2, 'weight_decay': 1e-2}
        args.w2_model_args = {'emb_dim': 256, 'learning_rate': 1e-2, 'weight_decay': 1e-2}
        args.imputation_model_args = {'learning_rate': 1e-2, 'weight_decay': 1e-5}
    else: 
...
def train_and_eval(train_data, unif_train_data, val_data, test_data, device = 'cuda', 
        ...
        w1_model_args: dict = {'emb_dim': 10, 'learning_rate': 0.1, 'weight_decay': 0.1}, 
        w2_model_args: dict = {'emb_dim': 10, 'learning_rate': 0.1, 'weight_decay': 0.1}, 
    ...
    base_optimizer = torch.optim.Adam(base_model.params(), lr=base_model_args['learning_rate'], weight_decay=base_model_args['weight_decay']) 
    ...
    w1_model = MF(n_user, n_item, dim= w1_model_args['emb_dim'], dropout=0).to(device)
    w1_optimizer = torch.optim.Adam(w1_model.parameters(), lr=w1_model_args['learning_rate'], weight_decay=w1_model_args['weight_decay'])

    w2_model = MF(n_user, n_item, dim= w2_model_args['emb_dim'], dropout=0).to(device)
    w2_optimizer = torch.optim.Adam(w2_model.parameters(), lr=w2_model_args['learning_rate'], weight_decay=w2_model_args['weight_decay']) 
    ...
    for epo in range(training_args['epochs']):
        for u_batch_idx, users in enumerate(train_loader.User_loader): 
            for i_batch_idx, items in enumerate(train_loader.Item_loader): 
                ...
                loss_f = loss_f_obs + base_model_args['imputaion_lambda'] * loss_f_all 
                ...
                # update w1 and w2 model
                y_hat_all = base_model(users_all, items_all)
                y_hat_all_detach = torch.detach(y_hat_all)
                cost_all = none_criterion(y_hat_all_detach, impu_all)
                y_hat_obs = base_model(users_train, items_train)
                y_hat_obs_detach = torch.detach(y_hat_obs)
                cost_obs = none_criterion(y_hat_obs_detach, y_train)
                y_hat_unif = base_model(users_unif, items_unif)
                y_hat_unif_detach = torch.detach(y_hat_unif)
                cost_unif = sum_criterion(y_hat_unif_detach, y_unif) 
                mar_loss = (1/len(y_unif)) * cost_unif

                w1_model.train()
                w1 = w1_model(users_all, items_all) # compute weight1
                w1 = torch.softmax(w1, dim=0) 

                w2 = w2_model(users_train, items_train) # compute weight
                w2 = torch.softmax(w2, dim=0)
                w2_detach = torch.detach(w2)  

                loss_all1 = (1/len(w1)) * torch.sum(w1*cost_all)        
                loss_obs1 = (1/len(w2_detach)) *torch.sum(w2_detach * cost_obs)    
                ...
                loss_all2 = (1/len(w1_detach)) * torch.sum(w1_detach * cost_all)
                loss_obs2 = (1/len(w2)) * torch.sum(w2 * cost_obs) 
                ...
                base_model.train()
                w1 = w1_model(users_all, items_all) 
                w1 = torch.softmax(w1, dim=0)
                w1_detach = torch.detach(w1) 

                w2 = w2_model(users_train, items_train) 
                w2 = torch.softmax(w2, dim=0)
                w2_detach = torch.detach(w2)   
                ...
                loss_all = torch.sum(cost_all * weight2 * w1_detach)

                # observation
                y_hat_obs = base_model(users_train, items_train)
                cost_obs = none_criterion(y_hat_obs, y_train)
                loss_obs = torch.sum(cost_obs * weight1 * w2_detach)
...
if __name__ == "__main__": 
    ...
    train_and_eval(train, unif_train, validation, test, device, base_model_args = args.base_model_args, w1_model_args = args.w1_model_args, w2_model_args = args.w2_model_args,
