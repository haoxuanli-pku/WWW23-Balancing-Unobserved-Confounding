...
def para(args): 
    if args.dataset == 'music': 
        args.training_args = {'batch_size': 1024, 'epochs': 300, 'patience': 30, 'block_batch': [6000, 500]}
        args.base_model_args = {'emb_dim': 10, 'learning_rate': 1e-5, 'weight_decay': 1}
        args.weight1_model_args = {'emb_dim': 10, 'learning_rate': 1e-6, 'weight_decay': 1}
        args.weight2_model_args = {'emb_dim': 10, 'learning_rate': 1e-9, 'weight_decay': 1}
        args.imputation_model_args = {'emb_dim': 2, 'learning_rate': 1e-5, 'weight_decay': 1}
    else: 
    ...
    for epo in range(early_stopping.max_epochs):
        for u_batch_idx, users in enumerate(train_loader.User_loader): 
            for i_batch_idx, items in enumerate(train_loader.Item_loader): 
                ...
                mar_loss = torch.sum((1/len(y_unif)) * e_unif)
