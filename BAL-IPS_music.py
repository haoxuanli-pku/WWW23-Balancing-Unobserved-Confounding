...
def para(args): 
    if args.dataset == 'music': 
        args.training_args = {'batch_size': 1024, 'epochs': 500, 'patience': 30, 'block_batch': [6000, 500]}
        args.base_model_args = {'emb_dim': 10, 'learning_rate': 1e-3, 'weight_decay': 10}
        args.weight_model_args = {'emb_dim': 10, 'learning_rate': 1e-1, 'weight_decay': 1e-2}
    else: 
...
def train_and_eval(train_data, unif_train_data, val_data, test_data, device = 'cuda', 
    ...
    weight_optimizer = torch.optim.SGD(weight_model.parameters(), lr=weight_model_args['learning_rate'], weight_decay=0)
    ...
    for epo in range(early_stopping.max_epochs):
        for u_batch_idx, users in enumerate(train_loader.User_loader): 
            for i_batch_idx, items in enumerate(train_loader.Item_loader): 
                ...
                mnar_loss = torch.sum(inverse_p * w * e_train)
                ...
                loss_weight_model = 1e-1 * sum_criterion(mnar_loss,mar_loss) + 10 * torch.dot(w,torch.log(w)) + weight_model_args['weight_decay'] * weight_model.l2_norm(users_train, items_train)
                ...
                loss = torch.sum(inverse_p * w_detach * cost) + model_args['weight_decay'] * model.l2_norm(users, items) 
