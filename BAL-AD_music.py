...
def para(args): 
    if args.dataset == 'music': 
        ...
        args.base_model_args = {'emb_dim': 10, 'learning_rate': 1e-4, 'imputaion_lambda': 1e-1, 'weight_decay': 1}
        args.weight1_model_args = {'learning_rate': 1e-2, 'weight_decay': 1e-2}
        args.weight2_model_args = {'learning_rate': 1e-3, 'weight_decay': 1e-3}
        args.w1_model_args = {'emb_dim': 10, 'learning_rate': 1e-5, 'weight_decay': 1}
        args.w2_model_args = {'emb_dim': 10, 'learning_rate': 1e-5, 'weight_decay': 1}
        args.imputation_model_args = {'learning_rate': 1e-1, 'weight_decay': 1e-2}
    else: 
...
def train_and_eval(train_data, unif_train_data, val_data, test_data, device = 'cuda', 
        ...
        w1_model_args: dict = {'emb_dim': 4, 'learning_rate': 0.1, 'weight_decay': 0.1}, 
        w2_model_args: dict = {'emb_dim': 4, 'learning_rate': 0.1, 'weight_decay': 0.1}, 
    ...
    w1_optimizer = torch.optim.SGD(w1_model.parameters(), lr=w1_model_args['learning_rate'], weight_decay=0)

    w2_model = NCF(n_user, n_item, dim= w2_model_args['emb_dim'], dropout=0).to(device)
    w2_optimizer = torch.optim.SGD(w2_model.parameters(), lr=w2_model_args['learning_rate'], weight_decay=0) 
    ...
    for epo in range(training_args['epochs']):
        for u_batch_idx, users in enumerate(train_loader.User_loader): 
            for i_batch_idx, items in enumerate(train_loader.Item_loader): 
                ...
                loss_all1 = torch.sum(w1*cost_all)        
                loss_obs1 = torch.sum(w2_detach * cost_obs)    
                mnar_loss1 = loss_all1 + loss_obs1
                loss_w1 = 1e-1 *sum_criterion(mnar_loss1,mar_loss) + 10 * torch.dot(w1,torch.log(w1)) + w1_model_args['weight_decay'] * w1_model.l2_norm(users_train, items_train)           
                ...
                loss_all2 = torch.sum(w1_detach * cost_all)
                loss_obs2 = torch.sum(w2 * cost_obs) 
                mnar_loss2 = loss_all2 + loss_obs2 
                loss_w2 = 1e-1 * sum_criterion(mnar_loss2,mar_loss) + 10 * torch.dot(w2,torch.log(w2)) + w2_model_args['weight_decay'] * w2_model.l2_norm(users_train, items_train)           
